...
– ¿Por qué cambia el valor de y tras la llamada a sum()?
	Cambia porque esta vez, en vez de pasar la variable se pasa un puntero de la variable que al ser modificado en la funcion sum, cambia en la variable original

– ¿Por qué en ocasiones se usa el operador ‘.’ y en otras ‘->’ para acceder a los campos de una estructura?
	Porque cuando un puntero apunta a una estructura, para accedfer a los valores de la misma se usa la flecha en vez de el punto
